...
test = [("There is a duck in this text", True), ("There is nothing here", False)]
...
### Simplify Mocking External Services with pytest-mock

```{python tags=c("hide-cell")}
# !pip install pytest pytest-mock
```

Testing code that interacts with external services, such as databases, is challenging because the behavior of these services might vary, introducing inconsistencies and making tests unreliable.
...
The example above shows how unmocked tests can fail due to changes in the database content or configuration.

pytest-mock provides the `mocker` fixture, a thin wrapper around the `mock` package, making it easier to mock external services and ensure predictable test behavior.
...
1. **Mock Connection**: Replace `psycopg2.connect` with a mock object to avoid actual database interactions.
2. **Return Value**: Simulate the query result by defining the return value for `fetchone`.
3. **Assertions**: Check that the mock connection and cursor were used with the expected parameters.

Outputs:

- Test passes without requiring a real database.
- Behavior is controlled and deterministic.

[Link to pytest-mock](https://pypi.org/project/pytest-mock/)
...
```{python}
from dirty_equals import IsList, IsNow, IsPartialDict, IsStr, IsTrueLike
```
...
def test_query_results(postgresql):
    ...
    with postgresql.cursor() as cur:
        ...
        cur.execute(
            "INSERT INTO test_table (name) VALUES ('John'), ('Jane'), ('Alice');"
        )
